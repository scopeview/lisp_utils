...
(lisp-unit2:define-test utils-test::filter--make-filter
    (:tags '(utils-test::tag-filter))
  (let* ((pattern "hi")
	 ;; (input-1 nil)
	 (input-2 "")
	 (input-3 "h")
	...
	(destructuring-bind (status list) (funcall (make-filter matcher) streamer)
...
(lisp-unit2:define-test utils-test::filter--define-filter
    (:tags '(utils-test::tag-filter))
  (let* ((pattern "hi")
	 ;; (input-1 nil)
	 (input-2 "")
    ...
    (define-filter test-filter-fun matcher)

    ;; (lisp-unit2:assert-eql 'partial-match (match pattern input-1))
    (with-input-from-string (s input-2)
      (let ((streamer (make-instance 'cstreamer :source s)))
	(destructuring-bind (status list) (test-filter-fun streamer)
	  (lisp-unit2:assert-eql 'not-filtered status)
	  (lisp-unit2:assert-eql nil list)
	  (lisp-unit2:assert-eql nil (read-char s nil nil)))))
